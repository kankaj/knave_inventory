...
function sheet(rows: (string | { wound: string })[], odolnost = 1): Character {
  const character = createCharacter()
  character.abilities.odolnost = odolnost
  rows.forEach((row, index) => {
    character.slots[index] =
      typeof row === 'string'
        ? { text: row, wound: false }
        : { text: row.wound, wound: true }
  })
  return character
}
...
describe('sendItems', () => {
  ...
  it('fills the recipient from the first open row', () => {
    ...
    expect(result.to.slots[1]).toEqual({ text: 'Olej', wound: false })
  })
